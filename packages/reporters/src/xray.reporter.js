...
export class XrayReporter {
  ...
  constructor(configProvider = null) {
    ...
    this.enabled = configProvider?.get('XRAY_ENABLED');
    this.xrayService = new XrayService(configProvider);
  }
  ...
  async onEnd() {
    ...
    const formattedResults = this.xrayService.formatResults(this.results);
    try {
      await this.xrayService.importExecution(formattedResults);
    } catch (error) {
  ...
  mapStatus(playwrightStatus) {
    return STATUS_MAP[playwrightStatus] || 'FAILED';
  }
}
