...
1.  Ensure you have run the scaffolder (`scaffold.sh`).
...
describe('User Profile Contract', () => {
  // Create a PactManager with configuration and initialize
  const pactManager = createPactManager(configManager);
  pactManager.setup('user-web-app', 'profile-service');
