...
To use the `XrayReporter` in your Playwright config:
```javascript
...
export default {
  reporter: [
    ['html'],
    [XrayReporter, { configProvider: configManager }],
  ],
};
```

To use the `XrayService` directly:
```javascript
import { createXrayService } from '@taflex/reporters';
import { configManager } from '@taflex/core';

const xrayService = createXrayService(configManager);
