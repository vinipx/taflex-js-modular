...
Unit tests are located in `tests/unit/` and follow the `*.spec.js` naming convention.
...
Checks that the correct driver is instantiated based on registered mode/provider keys and that custom drivers can be registered successfully.
